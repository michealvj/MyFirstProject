...
-(void)initializeViewComponents{
    ...
    self.PageBackgroundView = [[UIView alloc] initWithFrame:CGRectMake(0, self.frame.size.height - 48, self.frame.size.width, 48)];
    self.PageBackgroundView.backgroundColor = [UIColor colorWithRed:18.0f/255.0f green:82.0f/255.0f blue:190.0f/255.0f alpha:1.0f];
    [self addSubview:self.PageBackgroundView];
    
    //Page Control
    self.PageControl = [[UIPageControl alloc] initWithFrame:CGRectMake((self.frame.size.width - kPageControlWidth)/2, 0, kPageControlWidth, 48)];
    self.PageControl.currentPage = 0;
    
    self.PageControl.pageIndicatorTintColor = [UIColor colorWithRed:82.0f/255.0f green:143.0f/255.0f blue:243.0f/255.0f alpha:1.0f];;
    self.PageControl.currentPageIndicatorTintColor = [UIColor whiteColor];
    self.PageControl.enabled = NO;
    [self.PageBackgroundView addSubview:self.PageControl];
    
    //Get skipString dimensions
    NSString *skipString = NSLocalizedString(@"FINISHED >", nil);
    ...
    self.LeftSkipButton.frame = CGRectMake(10, self.frame.size.height - 48, 46, 48);
    ...
    self.RightSkipButton.frame = CGRectMake(self.frame.size.width - skipStringWidth - kLeftRightSkipPadding, self.frame.size.height - 48, skipStringWidth, 48);
    [self.RightSkipButton.titleLabel setFont:kSkipButtonFont];
    [self.RightSkipButton setTitle:@"NEXT >" forState:UIControlStateNormal];
    ...
    if (self.LanguageDirection == MYLanguageDirectionLeftToRight) {
        ...
        if (self.CurrentPanelIndex == (Panels.count)) {
            ...
            if ([(id)delegate respondsToSelector:@selector(introduction:didFinishWithType:)]) {
                [self hideWithFadeOutDuration:1.0];
...
-(void)skipIntroduction{
    if (self.CurrentPanelIndex==Panels.count-1) {
        ...
        [self changeToPanelAtIndex:self.CurrentPanelIndex+1];
    }
}
...
{
    if (self.CurrentPanelIndex==Panels.count-1) {
        [self.RightSkipButton setTitle:@"FINISHED >" forState:UIControlStateNormal];
    }
    else {
        [self.RightSkipButton setTitle:@"NEXT >" forState:UIControlStateNormal];
    }
